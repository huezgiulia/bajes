...
def initialize_emlikelihood_kwargs(opts):

    from ..obs.grb.filter import Filter
    from ..inf.prior import Prior
    ...
    if len(opts.lambdas) == 0:
        ...
        from ..obs.em import __photometric_bands__ as ph_bands
    ...
    l_kwargs = {}
    l_kwargs['approx']              = opts.em_approx
    l_kwargs['filters']             = Filter(opts.mag_folder, lambdas)
    ...
    priors = initialize_emprior(approx=kn_approx, bands=opts.bands,
    ...
    save_container(opts.outdir+'/em_obs.pkl', cont_kwargs)
    return l_kwargs, priors

def initialize_emprior(approx,
                       bands,
                       mej_bounds,
