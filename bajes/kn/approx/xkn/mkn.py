...
class MKN:
    ...
    def prep_inj_mag(
        ...
        if not self.mag:
            for lam in mag:
                mag[lam]["name"] = self.dic_filt[lam]["name"]  #QUA ASSOCIA NOMI A BANDE SULLA BASE DI UN FILTER DICTIONARY (OER ORA E' TELESCOPES)
        return mag
    ...
    def gen_inj_data(self, inj_dict):
        ...
        if inj_dict is None:
            ...
            measures = inj_dict["glob_params"]["t_scale"] == "measures"  #qua prende FALSE
