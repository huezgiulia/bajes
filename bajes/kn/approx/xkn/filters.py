...
def read_filter_measures(
    ...
    for lam in lams:
        ...
        mags_tot = np.asarray([])
        sigmas_tot = np.asarray([])                                                      #   QUA E' UN CASINOOOO !!! -->  CAPIRE SE HAI FATTO LA COSA PIU' SENSATA  !!! #
        for fname, mag_type in zip(dic_filt[lam]["filename"], dic_filt[lam]["type"]):
            # time, mag, sigma
            times, mags, sigmas = np.loadtxt(               #    QUESTO TIMES E' IN SECONDI (GPS TIME + SECONDS)!
                os.path.join(filter_data_path, fname), unpack=True, ndmin=1
            )
            #print('t_min', t_min)
            #print('t_max', t_max)

            #### DA FARE MEGLIO ASSOLUTAMENTE !!! ####

            # CONVERTO T_MIN E MAX IN SECONDI! (PER MENTENERE ARRAY TEMPI IN SECONDI COME NEI DATI -> PIU' COMPATIBILITA' CON BAJES(?))
            t_min_sec = (t_min * 24 * 60 * 60) + 1187008857
            t_max_sec = (t_max * 24 * 60 * 60) + 1187008857

            # oppure

            # CONVERTO TIME IN GIORNI (BISOGNA FARE UN IF PER QUANDO FARLO E QUANDO NON SERVE!)
            # times = (times - 1187008857) / 60 / 60 / 24

            #print('times', times)
            #print('t_min', t_min_sec)
            #print('t_max', t_max_sec)


            #print('mags', mags)
            t_mask = np.logical_and(times > t_min_sec, times < t_max_sec)
        ...
        #print('len(times_tot)', len(times_tot))
        if len(times_tot):
            measures[lam] = {
                "time": times_tot,
                "mag": mags_tot,
            ...
            #print('ELSE')
            del dic_filt[lam]
        #print(print('dic_filt in fondo a funzione read_filter_measures', dic_filt))
