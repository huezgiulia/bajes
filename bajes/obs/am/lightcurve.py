...
__approx_dict__ = { ### TIME-DOMAIN
                    # funcs
                    'am_afterglow':                {'path': 'bajes.obs.am.approx.afterglowpy_model.afterglow_wrapper',
                                                             'type': 'fnc'},
                  }
...
def __get_lightcurve_generator__(approx, times, nu, **kwargs):
    ...
    if this_light['type'] == 'fnc':
        ...
        raise AttributeError("Unable to define method for grb lightcurve generator. Check bajes.obs.am.lightcurve.__approx_dict__")

    return light_func


class AM_Lightcurve(object):
    """
        Astrometric Lightcurve object
    """
    ...
    def compute_centroid_motion(self, params): 
        dict_centroid = {}
        for n in self.nu.keys():
            centroid    =  self.light_func(self.times, self.nu[n], params)
            dict_centroid[n] = centroid
